...
chi_T = (alpha+alpha**2*T0)/(rho0*hcapa)

if (compressible) then
temp_surf = 0.091*T0*0
else
temp_surf=0.d0
end if 
...
write(*,*) 'initial min/max T',minval(T),maxval(T), sum(T)/np
!==============================================!
...
! open(unit=2,file="B.dat")
! do i=1,np
!   do j=1,np
!     write(1,'(5f20.10)',advance='no') A_T(i,j)

!   end do
!   write(1,*) " "
!     write(2,'(5f20.10)') B_T(i)
! end do
