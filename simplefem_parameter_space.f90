...
!Argument 5 : MUMPS_T                          !
!Argument 6 : MUMPS_VP                         !
!Argument 7 : Di Switch                        !
!Argument 8 : Ra Switch                        !
!Argument 9 : Tsurf
...
real(8) kn,kn_1                                ! timesteps for BDF-2 scheme
...
integer power,MUMPS_T_int,MUMPS_VP_int

real(8) Vsurf
real(8) work_grav,T_el
real(8) vr,er

real(8), external :: rho_eos
real(8) rho_eos_el,drhodx_eos,drhody_eos
logical use_eos
integer Di_switch,Ra_switch

real(8) temp_surf
real(8), dimension(4) :: temp_ref 


call get_command_argument(9,arg)
read(arg, '(f4.0)') temp_surf
...
rho0=1.d0
hcapa=1.d3 !Massive performance loss associated with parameter
hcond=1.d0
alpha=1.d0
...
!Delete this later
!Variables to agree with Blankenbach et al.
...
rho0 = 4.d0*10**3
hcapa=1.25*10**3
hcond=1.d-6*rho0*hcapa
alpha=2.5*1.d-5
viscosity = 2.5*1.d19*rho0
gy=-10.d0


!ok, now this is the bit where you read in the command line bits


call get_command_argument(7,arg)
read(arg, '(i1)') Di_switch

call get_command_argument(8,arg)
read(arg, '(i1)') Ra_switch


select case(Di_switch)
case(1)
  alpha = -Di*hcapa/(Lx*gy)
case(2)
  gy = -Di*hcapa/(Lx*alpha)
case(3)
  hcapa = -alpha*gy*Lx/Di
end select

select case(Ra_switch)
  case(1)
    rho0 = sqrt(-Ra*viscosity*hcond/(alpha*gy*T0*Lx**3*hcapa))
  case(2)
    alpha = -Ra*viscosity*hcond/(rho0**2*gy*T0*Lx**3*hcapa)
  case(3)
    gy = -Ra*viscosity*hcond/(rho0**2*alpha*T0*Lx**3*hcapa) 
  case(4)
    T0 = -Ra*viscosity*hcond/(rho0**2*alpha*gy*Lx**3*hcapa) 
  case(5)
    hcapa = -Ra*viscosity*hcond/(rho0**2*alpha*gy*T0*Lx**3) 
  case(6)
    viscosity = -rho0**2*alpha*gy*T0*Lx**3*hcapa/(Ra*hcond)
  case(7)
    hcond = -rho0**2*alpha*gy*T0*Lx**3*hcapa/(Ra*viscosity)
end select
...
do iel=1,nel
   ...
   do jq=-1,1,2
      ...
      do k=1,m
         ...
         !rho2 = rho2 + N(k)*rho_eos(y(icon(k,iel)),rho0,chi_T,gy)
      ...
      if (use_eos) then
      fel(i1)=fel(i1)+N(i)*jcob*wq*rho_eos_el*gx
      fel(i2)=fel(i2)+N(i)*jcob*wq*rho_eos_el*gy
      else
...
!===============================================!
!======[Calculate nodal frictional heating]=====!
!===============================================!

!This is probably wrong and an evil hack, but whatever I'm a bit fucking desperate at this point
!dxv_nodal = dyu_nodal
...
do iel=1,nel
   ...
   do jq=-1,1,2
      ...
      do k=1,m
         ...
         !This is probably the better version of phi
         ...
         ! phi = phi - viscosity*4.d0/3.d0*(dxu_nodal(icon(k,iel))+dyv_nodal(icon(k,iel)))**2


         !This is an experimental version of phi that is unlikely to work

         ! phi = phi + viscosity*4.d0/3.d0*dNdx(k)*u(icon(k,iel))*dNdx(k)*u(icon(k,iel))
         ! phi = phi + viscosity*4.d0/3.d0*dNdy(k)*v(icon(k,iel))*dNdy(k)*v(icon(k,iel))
         ! phi = phi - viscosity*(dNdx(k)*v(icon(k,iel)) + dNdy(k)*u(icon(k,iel)))**2
         ! phi = phi - viscosity*4.d0/3.d0*dNdx(k)*u(icon(k,iel))*dNdy(k)*v(icon(k,iel))
      ...
      !if ((istep .eq. 1) .or. .not. BA) then
      if (.true.) then
      AelT=AelT+(M_T+KK*theta*dt)

      BelT=BelT+matmul(M_T-KK*(1.d0-theta)*dt,temp(1:4)) + F*dt + matmul(Kc,temp_ref(1:4))

      else
      !Currently just for BA, extend to others if successful
      !Result of a failed attempt to extend to different timestepping 
      !scheme, stolen from ASPECT

      AelT=AelT + (2*kn+kn_1)/(kn+kn_1)*M_T - kn*Kc

      BelT=BelT + (kn+kn_1)/kn_1*matmul(M_T,temp(1:4)) - kn**2/(kn_1*(kn+kn_1))*matmul(M_T,temp_2(1:4))
      BelT=BelT - kn*((1+kn/kn_1)*matmul(Ka,temp(1:4)) - kn/kn_1*matmul(Ka,temp_2(1:4))) 
      end if
      ! do k=1,m
...
!Store old temperature and timestep
...
do iel=1,nel
   ...
   do jq=-1,1,2
      ...
      if(use_eos) then
      work_grav = work_grav + alpha*rho_eos_el*gy*v_el*T_el*wq*jcob
      adiabatic_heating(iel) = alpha*rho_eos_el*gy*v_el*T_el*wq*jcob
      else
      work_grav = work_grav + alpha*rho0*gy*v_el*T_el*wq*jcob
      adiabatic_heating(iel) = alpha*rho0*gy*v_el*T_el*wq*jcob
    end if
      phi_elemental(iel) = phi
   end do
   end do
...
if (mod(istep,output_freq)==0) then
write(*,*) "Phi= ",e_sh/er
write(*,*) "<W>= ", work_grav/er
end if 
open(unit=12345,file='OUT/phi_elemental.dat',status='replace')
do iel=1,nel
  write(12345,*) iel, phi_elemental(iel)
end do
close(12345)
!==============================================!
!========[Compute Heat Flux]===================!
...
! do i=1,np
!   density(i) = rho_eos(y(i),rho0,chi_T,gy,Ly)
! end do
...
if (isnan(sum(T))) then
  ...
  write(*,*) "Temperature is NaN, aborting"
  stop
end if
